  ...
  <div class="cross-node" :class="'depth-' + depth">
    ...
    <div v-if="isLogical" class="cross-logical pl-3">
      ...
      <div class="cross-actions mt-2">
        ...
        <!-- SF-6 (DEC-REF-63): el botón "Agregar hoja de suma" NO se
             renderiza en Capa 3. El componente RENDERIZA read-only una
             hoja sum si viniera en datos (robustez), pero no la crea. -->
      </div>
    ...
    <!-- HOJA SUMA — READ-ONLY (SF-6 activa la edición) -->
    <div v-else-if="isSumLeaf" class="cross-sum pl-3">
      <p class="text-info small mb-1">
        <i class="tim-icons icon-alert-circle-exc"></i>
        Hoja de suma — <strong>edición en SF-6</strong> (DEC-REF-63).
      </p>
      <pre class="small text-muted mb-0">{{ JSON.stringify(value, null, 2) }}</pre>
    </div>
  ...
  methods: {
    ...
    onTypeChange(newType) {
      ...
      if (newType === 'AND' || newType === 'OR') {
        ...
        // El select ni siquiera muestra 'leafSum' salvo que ya lo sea;
        // esta rama es defensiva y preserva shape.
        next = this.value;
      }
