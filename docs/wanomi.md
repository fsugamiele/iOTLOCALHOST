...
## Sesión #75 — 2026-09-17 · Área 2 · DEC-REF-100 implementado por fases F1–F8 — lives del sim, Panel progresivo, recomendaciones, devices, templates editables, reglas en idioma usuario y generador de escenarios

**Foco.** Debate de diseño de Franco (registrado como **DEC-REF-100** en el corpus, commit `8fac197`, ANTES del código): revisar lives del simulador que no publican, selects de dispositivo/sitio, Panel lento y gráfico foto, recomendaciones invisibles/no configurables, Devices (sitio por ícono roto, saver duplicado), templates editables con tamaño, UI de reglas en idioma usuario con modo avanzado, y generador de escenarios por sitio. Orden firmado: F1 lives+filtro · F2 recomendaciones · F3 devices · F4 templates · F5 Panel · F6 asistente reglas · F7 modo avanzado · F8 generador.

**Implementado — 8 commits de código, uno por fase.**

- **F1 (`fe3e7ae`) — lives por owner + filtro de sitio.** `GET /simulator/devices` devuelve `userId` owner y `EquipmentCard.vue` suscribe `${owner}/${dId}/...` (espejo del patrón del layout, DEC-REF-38): la causa de los lives muertos era suscribir con el userId del CALLER con devices de dos owners ajenos. Filtro de sitio client-side ("Todos los sitios") en `simulator.vue`; `icon-flash`→`icon-light-3`.
- **F2 (`787edbc`) — recomendaciones visibles.** `recommendation` sumada al payload MQTT dashboard del edge (`notificationRouter.js`) y render en toast (`default.vue` con sufijo `→`), `EquipmentAlarms.vue`, `NocRecentAlarms.vue` y dropdown del navbar. El campo editable quedó para D-7 (F6/F7), no F2.
- **F3 (`92e5f97`) — devices.** Alta = nombre + template (afuera select de sitio y switch saver del alta; saver nace ON por backend; bind y toggle siguen en la tabla). Fix del ícono roto: `icon-pin-3`→`icon-pin` (inexistente en nucleo-icons.css) + auditoría de íconos en 11 pantallas sin faltantes.
- **F4 (`2a02ded`) — templates editables.** `PUT /template` nuevo con `buildWriteFilter` + guardas fail-closed: quitar variables en uso por devices asociados o por reglas del pack (D/C/S directas + hojas crossExpr vía `collectCrossLeafRefs`) → **409 con detalle**; userId/createdTime preservados. UI: botón Editar por fila carga la plantilla en el configurador (nombre, ficha, widgets — el tamaño `column` se edita con los mini-forms existentes), `saveTemplate` bifurca PUT/POST, cancelar edición, notify con el detalle del 409.
- **F5 (`2f374c4`) — Panel progresivo + gráfico vivo.** `GET /dashboard/noc?phase=vivo` responde el tramo VIVO sin disparar las 5 queries pesadas (guards lazy sobre promesas eager; `composeVivo` fuente única para fase viva y full): medido **135 ms** vs MISS pesado 20,3 s. `dashboard.vue` dispara viva+full en paralelo en la carga inicial (`_fullLoaded` evita que la viva pise datos completos); `sites`/`trendVariables` llegan null en la fase viva y solo esos bloques quedan en skeleton. `NocTrendChart.vue` gana auto-refresh silencioso 60 s (sin spinner, guard anti-solape, error transitorio no pisa data buena).
- **F6 (`435a274`) — asistente de reglas.** Wizard 4 pasos en `_packId.vue` (equipo → variable de la ficha con unidad → condición en lenguaje natural → importancia explicada + recomendación + nombre) para type D; ruleId/inferenceId autogenerados ocultos (slug único en el pack); "Opciones avanzadas" deriva al formulario clásico conservando lo cargado; editar una regla D abre el wizard prellenado, C/S/cross van directo al avanzado. Tabla legible: nombre + "qué hace" en idioma usuario + recomendación visible; ruleId/tipo como detalle secundario.
- **F7 (`6f547b7`) — modo avanzado.** Formulario clásico reorganizado: arriba lo esencial (nombre, importancia con labels de usuario, tipo, equipo, variable, **recomendación editable** — cierra el hueco de D-4 en el form); sección "Opciones avanzadas" desplegable con ruleId/inferenceId, tiempos (cooldown/grace) y configs de cross/C/S; se abre sola al elegir tipo no-D, al editar regla avanzada o al venir del wizard. `CrossExprNode.vue` en idioma usuario: "TODAS estas condiciones (AND)"/"CUALQUIERA de estas condiciones (OR)", "Condición de equipo", "Suma entre equipos", comparadores en español (OPERATOR_LABELS, capa de presentación — identidad lt/gt intacta), nota de comportamiento del grupo.
- **F8 (`b27032c`) — generador de escenarios.** Colección nueva `simscripts` (pasos segundo+equipo+variable+valor por sitio, cleanup `reset`/`hold`, límites 50 pasos/30 min). Backend en `simulator.js`: CRUD + duplicate + run/stop/active con validación shape y validación variable/valor contra template (idéntica al botón Aplicar) y sitio dentro del scope. **Reloj EN EL SERVIDOR** (`services/simScriptRunner.js`): un setTimeout por paso publicando `set_sensor` por `publishCommand` (mismo camino que Aplicar; proceso del simulador intacto), bloqueo de un guion activo por sitio (409), Detener sin cleanup, cleanup final `reset` a los equipos tocados. Riesgo firmado: restart del backend corta el guion sin cleanup (mitigado con Detener + Reset por equipo). `GET /simulator/scenarios` expone `steps` para el clone. UI: sección "Generador de escenarios" en `/demo/simulator` con select de sitio, lista (ejecutar/editar/duplicar/borrar), editor de renglones con valor tipado por variableType, barra de progreso con pasos marcados (polling 2 s solo durante ejecución) y bloqueo visual. Catálogo de 17 escenarios queda solo-lectura con **"Clonar y editar"** por escenario (expande `{at, set}` a pasos de guion sobre el equipo elegido en la tarjeta).

**Verificación (stack dev).** Build `docker_nuxt_build.yml` exit 0 por fase · strings medidos en chunks de `app/dist/_nuxt` · UI 200 por pantalla tocada · API 401 sin token (control). **F4**: smoke E2E (`/tmp/smoke_f4.js`+`f4b`): alta/edición 200 con rename + tamaño `col-12` + createdTime preservado, 400 deviceType inválido, **409 quitando `shelter_temp` de WN-SITE-SEC v2 con 4 devices asociados** (mensaje con variable y count), control sin quitar 200. **F5**: `phase=vivo` 135 ms con sites/trendVariables null y 5 alarmas; full MISS 20,3 s completo. **F8**: smoke E2E (`/tmp/smoke_f8.js`): 400 sin pasos, 400 variable fuera de template, create/list/run/active/doble-run-409/stop/fin-solo/duplicate/delete todo PASS; **verificado contra el sim vivo en el log: `set_sensor battery_beacons_count=111` y `=222` sobre CR00061/SEC** — el reloj del servidor mueve el simulador real.

**Declarado.**

1. **El sim estaba caído al llegar a F8** (pid muerto desde antes de la sesión; el primer smoke publicó al vacío). Restaurado con el arranque válido de `tools/apertura.sh` y repetida la verificación con sim vivo. El healthcheck de apertura no se corrió entre sesiones — mismo patrón declarado en #74 (ítem 3 de carry-over sigue vigente).
2. Sin click-through visual en browser (deuda arrastrada #72–#74: sin headless en el host; verificación por API + chunks + MQTT + log del sim).
3. MISS pesado del Panel medido en **20,3 s** (estimado 12 s en #53 — el dato creció); el render progresivo lo absorbe, pero la query pesada sigue siendo candidata a BACKLOG-PERF-1.

### Carry-over para #76, en orden

1. Click-through visual del wizard de reglas, modo avanzado, edición de templates y generador de escenarios.
2. Push pendiente de los 10 commits de la sesión (`8fac197` corpus · `fe3e7ae` F1 · `787edbc` F2 · `92e5f97` F3 · `2a02ded` F4 · `2f374c4` F5 · `435a274` F6 · `6f547b7` F7 · `b27032c` F8 · asiento de esta sesión) — requiere orden explícita de Franco.
3. Deudas vivas de #74 ítem 5 (booleanDwell/equipmentAlarms con datos reales, credenciales fuera de /tmp, ficha `cummins-pcc` en prod, archivos sin trackear) + gates propios vigentes.
